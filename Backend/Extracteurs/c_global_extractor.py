...
# [correctif] Sur certaines lignes, pdfplumber ne trouve AUCUN espace
# entre le code produit et la marque (ex. "125MCDSTPMEALIA" au lieu de
# "125MCDSTPME ALIA") -- meme extract_words() les fusionne en un seul
# mot, car les caracteres se touchent reellement dans le PDF source a
# cet endroit precis. Sans ce correctif, `ligne.split()` ne renvoie que
# 11 tokens au lieu de 12 et `parse_produit()` rejette la ligne
# ENTIERE silencieusement (`if len(m) < 12: return None`).
#
# On repere la marque connue en suffixe du token colle pour re-separer
# le code. A completer si une nouvelle marque non listee apparait (un
# warning est loggue dans ce cas, la ligne est alors quand meme rejetee
# comme avant -- mieux vaut la signaler que la deviner a tort).
MARQUES_CONNUES = [
    "Hacendado", "Hacendad", "Delicius", "ALIA", "SEASON",
]


def _split_code_marque(token):
    """Tente de separer un token "code+marque" colle sans espace, en
    reconnaissant une marque connue en suffixe. Retourne (code, marque)
    ou (token, None) si aucune marque connue ne matche."""
...
class CoutGlobalExtractor:
    ...
    def parse_produit(self, ligne):
        ...
        if len(m) == 11:
            # [correctif] code et marque colles sans espace
            # (ex. "125MCDSTPMEALIA") -> tentative de separation.
            code, marque = _split_code_marque(m[0])
            if marque is None:
                logger.warning(
                    f"C.Global : impossible de separer code/marque sur "
                    f"le token colle '{m[0]}' (aucune marque connue "
                    f"reconnue en suffixe) -> ligne ignoree. Ajouter la "
                    f"marque a MARQUES_CONNUES si c'est une marque reelle."
                )
                return None
            valeurs = m[1:]
        else:
