...
# ==========================================
# 8. EXEMPLE D'UTILISATION (a adapter a tes vrais chemins de PDF)
# ==========================================
if __name__ == "__main__":

    # --- par_article + son cout global ---
    with_par_article = ParArticleExtractor(r"d:\Cumarex_1\Backend\data_sources\par_article_1.pdf")
    import pdfplumber
    with pdfplumber.open(r"d:\Cumarex_1\Backend\data_sources\Par Article_2.pdf") as pdf:
        par_article_data = with_par_article.extract(pdf.pages[0])

    cg = CoutGlobalExtractor(r"d:\Cumarex_1\Backend\data_sources\C.Global.pdf")
    cg_result = cg.extraire()

    # --- resume.py (site a preciser toi-meme, point ouvert A) ---
    pdf_path=r"d:\Cumarex_1\Backend\data_sources\Résume_1.pdf"
    resume_extractor = ResumeExtractor(pdf_path)
    resume_result = resume_extractor.extract()

    # --- rendement ---
    rendement = RendementExtractor(r"D:\Cumarex_1\Backend\data_sources\Rendement.pdf")
    rendement_result = rendement.extract()

    # --- mod S1 / S2 ---
    mod_s1 = ModExtractor(r"d:\Cumarex_1\Backend\data_sources\Mod S1.pdf").extract()
    mod_s2 = ModExtractor_2(r"d:\Cumarex_1\Backend\data_sources\Mod S2.pdf").extract()

    # [correctif] Verification croisee des 3 dates avant ingestion :
    # par_article.date_production (= date_reception), C.Global.date et
    # Rendement.date doivent correspondre a la MEME journee -- sinon on
    # risque de joindre des donnees de jours differents sous un seul
    # id_production. Ce log reste utile comme alerte precoce et globale
    # (les 3 sources d'un coup), mais ce n'est PLUS le seul garde-fou :
    # la verification qui bloque reellement la fusion par_article/C.Global
    # est maintenant faite DANS le pipeline (_find_cout_global_row, via
    # date_cout_global transmis a execute_etl ci-dessous), donc elle
    # s'applique aussi si execute_etl() est appele depuis un autre
    # contexte (API, scheduler...) et pas seulement depuis ce bloc de demo.
    date_par_article = _parse_date_ddmmyyyy(par_article_data.get("date_production"))
    date_cglobal = cg_result.get("date")
    date_rendement = rendement_result.get("date")

    dates_trouvees = {
        d for d in (date_par_article, date_cglobal, date_rendement) if d is not None
    }
    if len(dates_trouvees) > 1:
        logger.warning(
            f"Dates incoherentes entre sources : par_article={date_par_article}, "
            f"C.Global={date_cglobal}, Rendement={date_rendement}. "
            f"Verifier que les 3 PDF correspondent bien a la meme journee."
        )

    execute_etl(
        par_article_dict=par_article_data,
        df_cout_global_produits=cg_result["produits"],
        date_cout_global=date_cglobal,
        resume_data=resume_result,
        resume_site_code="S1",  # <-- A CONFIRMER
        df_entrees_rendement=rendement_result["df_entrees"],
        df_summary_rendement=rendement_result["df_summary"],
        # [correctif] Desormais issue directement de RendementExtractor
        # (qui l'extrait lui-meme), avec repli sur celle de par_article
        # si jamais Rendement.pdf ne la fournit pas (mise en page
        # inattendue -> extraire_date_rapport() renvoie None).
        rendement_context_date=date_rendement or date_par_article,
        df_mod_s1=mod_s1,
        df_mod_s2=mod_s2,
        # strict_dates=True,  # decommenter si tu preferes bloquer
        # completement l'ETL (rollback) en cas d'incoherence de date,
        # plutot que d'ignorer silencieusement les colonnes C.Global.
    )
