...
# [v13] Ils ne sont plus pour autant perdus : upsert_mod_long_format() les
# transfere desormais a part dans la table dediee `mod_totaux_espece`.
...
def replace_ecarts_couts_postes(session, fk_resolver, id_production, df_table_couts):
    ...
    ParArticleExtractor.extract_table_couts() -- le tableau COMPLET
    "Reel vs Stnd" par poste de la fiche par_article (poste, reel_0,
    stnd_0, cout_reel, cout_stnd, reel_1, stnd_1).

    [v13] Auparavant, seule la paire couts (cout_reel/cout_stnd) etait
    transferee en base ; les 2 autres paires Reel/Stnd du meme tableau
    (reel_0/stnd_0 = quantites, reel_1/stnd_1 = 2e paire de droite)
    etaient bien extraites par extract_table_couts() mais jetees ici
    sans jamais etre stockees. Elles sont desormais persistees a leur
    tour (voir migration_v13_totaux_mod_et_ecarts_quantites.sql), pour
    que la table "Reel vs Stnd" du par_article soit transferee dans son
    integralite, pas seulement sa colonne couts.
    ...
    [correctif] Les colonnes ecart_unitaire / pct_ecart / ecart_quantite /
    pct_ecart_quantite ont ete retirees de la table (calculs juges faux) :
    on ne les insere donc plus ici.
    """
    session.execute(
    ...
    # [correctif] Colonnes ecart_unitaire, pct_ecart, ecart_quantite et
    # pct_ecart_quantite supprimees de la table ecarts_couts_postes en
    # base (valeurs jugees fausses) -> retirees ici aussi de l'INSERT et
    # de tout calcul associe.
    query = text("""
        INSERT INTO ecarts_couts_postes (
            id_production, id_poste_cout,
            reel_unitaire, stnd_unitaire,
            quantite_reelle, quantite_stnd,
            reel_secondaire, stnd_secondaire
        )
        VALUES (
            :id_production, :id_poste_cout,
            :reel, :stnd,
    ...
    for row in df_table_couts.to_dict(orient="records"):
        ...
        # -- Paire couts (deja geree avant v13) --
        reel = float(row.get("cout_reel") or 0)
        stnd = float(row.get("cout_stnd") or 0)

        # -- [v13] Paire quantites (reel_0 / stnd_0) --
        # None si la cellule etait vide/"-" dans le PDF (contrairement a
        # la paire couts ci-dessus, on ne force pas a 0 : une quantite
        # absente n'est pas une quantite nulle, cf. meme logique que
        # mod_durees_espece_poste plus bas dans ce fichier).
        qte_reelle = row.get("reel_0")
        qte_stnd = row.get("stnd_0")

        # -- [v13] 2e paire Reel/Stnd (reel_1 / stnd_1) --
        reel_sec = row.get("reel_1")
        stnd_sec = row.get("stnd_1")
        ...
        session.execute(query, {
            ...
            "reel": reel,
            "stnd": stnd,
            "qte_reelle": qte_reelle if pd.notna(qte_reelle) else None,
            "qte_stnd": qte_stnd if pd.notna(qte_stnd) else None,
            "reel_sec": reel_sec if pd.notna(reel_sec) else None,
            "stnd_sec": stnd_sec if pd.notna(stnd_sec) else None,
        })
...
def get_or_create_resume_journalier(session, fk_resolver, date_production, reference_jour, site_code, poisson_bloc=None):
    ...
    [v12] site_code : depuis que le PDF Resume peut melanger des
    articles de plusieurs sites (aucune info site dans ce PDF), cette
    valeur n'est plus forcement une constante fournie par l'appelant :
    voir run_etl_resume(), qui la deduit desormais PAR REFERENCE/PAGE
    en croisant les codes articles de cette page avec le site connu de
    ces memes codes dans par_article du meme jour (site_par_code), et
    ne retombe sur une valeur fixe qu'en dernier recours.
    """
...
# ==========================================
# 6. MOD_S1 / MOD_S2 EXTRACTORS -> mod_communs_journaliers /
#    mod_durees_espece_poste / mod_totaux_espece (TOT/TOT2, v13)
# ==========================================
...
def upsert_mod_long_format(session, fk_resolver, df_mod, site_code):
    ...
    `mod_totaux_espece` (TOT/TOT2, v13).
    ...
    # On ne garde que les 4 vrais postes ET/FIL/EMB/SRTI dans
    # mod_durees_espece_poste : "TOT"/"TOT2" (sous-totaux par espece, cf.
    # POSTES_PRODUCTION_A_IGNORER) en sont ecartes pour ne jamais polluer
    # postes_production ni fausser un SUM(duree_min_kg) sur ET+FIL+EMB+SRTI
    # par un double comptage.
    #
    # [v13] Ces totaux ne sont plus pour autant perdus : ils sont desormais
    # transferes a part, dans la table dediee `mod_totaux_espece`
    # (date_production, id_site, id_type_poisson, code_total, valeur) --
    # voir migration_v13_totaux_mod_et_ecarts_quantites.sql.

    prod = df_mod[df_mod["categorie"] == "Production"]

    nb_totaux_transferes = 0
    nb_valeurs_absentes = 0

    query_totaux = text("""
        INSERT INTO mod_totaux_espece (
            date_production, id_site, id_type_poisson, code_total, valeur
        )
        VALUES (:date_prod, :id_site, :id_poisson, :code_total, :valeur)
        ON CONFLICT (date_production, id_site, id_type_poisson, code_total)
    ...
    for _, row in prod.iterrows():
        ...
        # ---------------------------------------------------------
        # [v13] TOT et TOT2 -> mod_totaux_espece (pas mod_durees_espece_poste)
        # ---------------------------------------------------------
        if service_key in POSTES_PRODUCTION_A_IGNORER:
            if pd.isna(row["valeur"]):
                # Meme logique que pour les vrais postes : une cellule
                # vide/"-"/"?" n'est pas un total de 0, on n'insere rien.
                nb_valeurs_absentes += 1
                continue

            date_prod = _parse_date_ddmmyyyy(row["date"])
            id_poisson = fk_resolver.get_or_create_poisson(row["type_poisson"])

            session.execute(query_totaux, {
                "date_prod": date_prod,
                "id_site": id_site,
                "id_poisson": id_poisson,
                "code_total": service_key,
                "valeur": row["valeur"],
            })
            nb_totaux_transferes += 1
            continue
    ...
    if nb_totaux_transferes:
        logger.info(
            f"mod_{site_code} : {nb_totaux_transferes} ligne(s) "
            f"'TOT'/'TOT2' (sous-totaux par espece) transferee(s) dans "
            f"mod_totaux_espece (non inserees dans mod_durees_espece_poste, "
            f"pour ne pas fausser un SUM() sur ET+FIL+EMB+SRTI)."
        )
...
def _codes_pour_reference(resume_data, reference):
    """[v12] Rassemble tous les codes articles (colonne "code") qui
    apparaissent pour une "reference"/page donnee du PDF Resume, en
    regardant table_codes (mod_par_code), couts_unitaires et
    couts_serie -- les 3 sources qui portent un code article resolu de
    la meme facon que par_article (voir fk_resolver.get_article_id).
    Utilise par run_etl_resume() pour deduire le site de cette page.
    """
    codes = set()
    for cle in ("mod_par_code", "couts_unitaires", "couts_serie"):
        df = resume_data.get(cle)
        if df is None or getattr(df, "empty", True):
            continue
        if "reference" not in df.columns or "code" not in df.columns:
            continue
        for code in df.loc[df["reference"] == reference, "code"].dropna():
            codes.add(str(code).strip().upper())
    return codes


def _deduire_site_reference(codes_reference, site_par_code, reference, site_defaut):
    """[v12] Determine le site d'une page/reference du PDF Resume en
    croisant ses codes articles avec `site_par_code` (deduit du
    par_article du meme jour -- voir execute_etl()).

    - 1 seul site trouve parmi les codes de la page -> on l'utilise.
    - 0 site trouve (aucun code connu / pas de par_article ce jour-la)
      -> repli sur `site_defaut` (le resume_site_code fourni a
      lancer_etl_complet, souvent encore necessaire en debut de mois
      ou si aucun Par Article n'a ete importe pour cette date).
    - Plusieurs sites differents trouves sur la MEME page -> anomalie
      (une page/reference du PDF Resume represente normalement un
      seul lot de production, donc un seul site) : on logue une alerte
      et on retombe aussi sur `site_defaut`, plutot que de deviner.
    """
    sites_trouves = {site_par_code[c] for c in codes_reference if c in site_par_code}

    if len(sites_trouves) == 1:
        site = sites_trouves.pop()
        logger.info(f"resume.py : site '{site}' deduit pour la reference '{reference}' via croisement avec par_article.")
        return site

    if len(sites_trouves) > 1:
        logger.warning(
            f"resume.py : reference '{reference}' associee a PLUSIEURS sites differents "
            f"({sites_trouves}) selon par_article -> anomalie, repli sur le site par defaut "
            f"'{site_defaut}'. A verifier manuellement (page melangeant 2 sites ?)."
        )
        return site_defaut

    logger.info(
        f"resume.py : aucun code de la reference '{reference}' trouve dans par_article du jour "
        f"-> repli sur le site par defaut '{site_defaut}'."
    )
    return site_defaut


def run_etl_resume(session, fk_resolver, resume_data, site_code, site_par_code=None):
    """Ingestion complete pour resume.py.

    site_code : valeur de REPLI (plus une valeur imposee -- voir [v12]
    ci-dessous), utilisee quand le site n'a pas pu etre deduit.

    site_par_code : [v12] dict {code_article_normalise: site_normalise}
    construit par execute_etl() a partir du/des par_article du meme
    import (memes codes articles que resume.py, voir
    _codes_pour_reference / _deduire_site_reference). Permet de gerer
    un PDF Resume dont les articles appartiennent a des sites
    differents (aucune info site dans ce PDF), en deduisant le site
    PAR REFERENCE/PAGE plutot que d'en imposer un seul pour tout le
    fichier. Si None/vide (ex: Resume importe seul, sans Par Article
    ce jour-la), le comportement d'origine est conserve : site_code
    est applique tel quel a toutes les pages.
    ...
    site_par_code = site_par_code or {}
    ...
    for _, info_row in info.iterrows():
        ...
        codes_reference = _codes_pour_reference(resume_data, reference)
        site_reference = _deduire_site_reference(codes_reference, site_par_code, reference, site_code)
...
def execute_etl(
    ...
    resume_site_code : [v12] N'EST PLUS APPLIQUE TEL QUEL A TOUT LE PDF
    Resume. Le PDF Resume peut contenir plusieurs articles repartis sur
    plusieurs sites, sans aucune info de site dans le PDF lui-meme. Le
    site de chaque page/reference est desormais DEDUIT automatiquement
    en croisant ses codes articles avec le site connu de ces memes
    codes dans par_article_list/par_article_dict du meme import (voir
    site_par_code plus bas, et run_etl_resume()). resume_site_code
    devient une valeur de REPLI, utilisee uniquement quand la
    deduction echoue (aucun Par Article fourni ce jour-la, ou code
    absent de par_article).
    """
    session = Session()
    ...
    try:
        ...
        # [v12] site_par_code : {code_article_normalise: site_normalise},
        # construit AVANT la boucle d'ingestion pour capturer le site tel
        # qu'imprime sur chaque fiche par_article (avant normalisation
        # CX0/CX1 -> S1/S2 faite plus bas par run_etl_par_article), et
        # reutilise ensuite par run_etl_resume() pour deduire le site de
        # chaque page du PDF Resume par croisement de codes -- voir
        # run_etl_resume() / _deduire_site_reference(). Un meme code
        # associe a 2 sites differents dans ce batch (rare mais possible
        # si le meme article tourne sur S1 et S2 le meme jour) est
        # signale et retire de la table -> _deduire_site_reference()
        # traitera alors ce code comme "site inconnu" (repli sur defaut)
        # plutot que de choisir arbitrairement l'un des deux.
        site_par_code = {}
        codes_ambigus = set()
        for article_dict in articles_a_traiter:
            code = article_dict.get("code_produit")
            site = normalize_site_code(article_dict.get("site"))
            if not code or not site:
                continue
            code_norm = str(code).strip().upper()
            if code_norm in codes_ambigus:
                continue
            if code_norm in site_par_code and site_par_code[code_norm] != site:
                logger.warning(
                    f"site_par_code : le code '{code_norm}' apparait avec 2 sites "
                    f"differents dans ce batch ({site_par_code[code_norm]} et {site}) "
                    f"-> retire du mapping, traite comme site inconnu pour resume.py."
                )
                del site_par_code[code_norm]
                codes_ambigus.add(code_norm)
                continue
            site_par_code[code_norm] = site
        ...
        if resume_data is not None:
            run_etl_resume(session, fk_resolver, resume_data, resume_site_code, site_par_code=site_par_code)
...
def lancer_etl_complet(
    ...
    resume_site_code : [v12] valeur de REPLI pour le site des pages du
    PDF Resume, utilisee uniquement si le site n'a pas pu etre deduit
    par croisement avec path_par_article du meme appel (voir
    execute_etl() / run_etl_resume()). Fournir aussi path_par_article
    dans le meme appel est donc fortement recommande des que le Resume
    peut contenir des articles de plusieurs sites -- sinon ce parametre
    reste applique tel quel a toutes les pages, comme avant.
    """
    ...
    if path_rendement:
        ...
        # [correctif] RendementExtractor.extract() renvoie les clés
        # "df_entrees" / "df_summary" (voir rendement_extractor.py), pas
        # "entrees" / "summary". Avec les anciennes clés, .get() renvoyait
        # toujours None -> df_entrees_rendement / df_summary_rendement
        # valaient toujours None -> upsert_lots_poisson() retournait
        # immediatement sans rien inserer, SANS LA MOINDRE ERREUR : les
        # donnees Rendement n'etaient donc jamais chargees en base.
        df_entrees_rendement = rendement_data.get("df_entrees")
        df_summary_rendement = rendement_data.get("df_summary")
