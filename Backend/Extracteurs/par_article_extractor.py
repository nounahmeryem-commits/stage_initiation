...
class ParArticleExtractor:
    ...
    def table_couts_combined(self, articles):
        ...
        for article in articles:
            ...
            frames.append(df)
        if not frames:
            return pd.DataFrame(columns=["page", "ordre_fabrication", "poste", "cout_reel", "cout_stnd"])
...
if __name__ == "__main__":
    pdf_path = r"d:\Cumarex\Proposed project\Proposed project\Production\2026\06_Juin\08_06_2026\Par Article.pdf"  # <- adapte ce chemin à ton PDF
    extractor = ParArticleExtractor(pdf_path)

    # Extraction de TOUTES les fiches (une par page) du PDF
    articles = extractor.extract_pdf()
    print(f"{len(articles)} fiche(s) extraite(s) du PDF.\n")

    for article in articles:
        print(f"--- Fiche page {article['page']} "
              f"({article.get('ordre_fabrication')} / "
              f"{article.get('espece')} / {article.get('recette')}) ---")
        for cle, valeur in article.items():
            if cle == "table_couts":
                continue
    ...
    # Résumé : une ligne par fiche, toutes fiches confondues
    df_resume = extractor.articles_to_dataframe(articles)
    print("=== Résumé toutes fiches ===")
    print(df_resume.to_string(index=False))

    # Détail des coûts : toutes les fiches empilées, avec identifiant
    df_couts = extractor.table_couts_combined(articles)
    print("\n=== Table des coûts, toutes fiches ===")
    print(df_couts.to_string(index=False))