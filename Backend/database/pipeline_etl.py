...
def _find_cout_global_row(df_produits, code_produit, site_code=None):
...
def run_etl_par_article(session, fk_resolver, par_article_dict, df_cout_global_produits):
    ...
    if not cout_global_row:
        logger.warning(
            f"Aucune ligne C.Global trouvee pour le code '{par_article_dict.get('code_produit')}' "
...
def execute_etl(
    ...
    try:
        ...
        if par_article_dict is not None:
            run_etl_par_article(session, fk_resolver, par_article_dict, df_cout_global_produits)
...
if __name__ == "__main__":
    ...
    # id_production. On logge un avertissement plutot que de bloquer,
    # au cas ou un decalage soit parfois legitime (a evaluer au cas par
    # cas si le message apparait).
